...
/* So this function is called from the led_task function that is called when the user selects the sLedEffect from menu task */
void led_effect(int n)
{
    led_effect_stop();/** Stops the previous LED effect. **/
    xTimerStart(handle_led_timer[n-1],portMAX_DELAY);/* We only start the LED timer for the LED effect the user chooses. */
}
...
{
	/* The effect is toggle all LEDs after 500ms, here we use static flag to xor with itself to get the toggling done*/
	static int flag = 1;
